...
namespace SnakeGame
{
    public class SnakeGameView2x : Game, View
    {
        private int _w;
        private int _h;
        ...
        public SnakeGameView2x(int w, int h)
        {
            _w = w;
            _h = h;
            try
        ...
        protected override void LoadContent()
        {
            ...
            try
            {
                using (var stream = TitleContainer.OpenStream("Content/black.png"))
                {
                    snakeTile = Texture2D.FromStream(this.GraphicsDevice, stream);
                }
                using (var stream = TitleContainer.OpenStream("Content/gray.png"))
                {
                    foodTile = Texture2D.FromStream(this.GraphicsDevice, stream);
                }
                using (var stream = TitleContainer.OpenStream("Content/white.png"))
                {
